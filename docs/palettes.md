...
<script type="module">
    ...
    const palettes = [        
        ...
        { label: "Canmore legend", colours: chroma.canmore.map(item => item.colour) },
        // https://pastmap.org.uk/map
        // color values obtained using Chrome eyedropper extension on legend
        { label: "Pastmap legend", colours: chroma.pastmap.map(item => item.colour) },
        // Peter's 12 colour palettes
        { label: "Peter's legend1", colours: chroma.peters1.map(item => item.colour) },
        { label: "Peter's legend2", colours: chroma.peters2.map(item => item.colour) }        
    ]    
